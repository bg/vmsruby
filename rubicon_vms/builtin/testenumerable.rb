...
class TestEnumerable < Rubicon::TestCase
  ...
  def generic_test_detect(method)
    ...
    assert_equal(nil, E().send(method) {|a| true  })
    assert_equal(nil, @a.send(method) {|a| false })
    assert_equal(2,   @a.send(method) {|a| a > 1 })
    assert_equal(6,   @a.send(method) {|a| a > 5 })
    assert_equal(10,  @a.send(method) {|a| a > 9 })
    assert_equal(nil, @a.send(method) {|a| a > 10 })
  end
  ...
  def test_max
    ...
    assert_raises(NoMethodError) { E(Object.new, Object.new).max }
    Version.less_than("1.9.0") do
      assert_raises(NoMethodError) { E(11,"22").max }
    end
    Version.greater_or_equal("1.9.0") do
      assert_raises(ArgumentError) { E(11,"22").max }
    end
  ...
  def test_min
    ...
    assert_raises(NoMethodError) { E(Object.new, Object.new).min }
    Version.less_than("1.9.0") do
      assert_raises(NoMethodError) { E(11,"22").min }
    end
    Version.greater_or_equal("1.9.0") do
      assert_raises(ArgumentError) { E(11,"22").min }
    end
  ...
  Version.greater_or_equal("1.6.8") do
    def test_zip
      assert_equal([%w(a 1), %w(b 2), %w(c 3), %w(d 4)],
                   E('a','b','c','d').zip(%w(1 2 3 4)))
      assert_equal([%w(a 1), %w(b 2)], E('a','b').zip(%w(1 2 3 4)))
      assert_equal([%w(a 1), %w(b 2), ['c', nil], ['d', nil]],
                   E('a','b','c','d').zip(%w(1 2)))
      assert_equal([[1], [2]], E(1, 2).zip)
      assert_equal([["a\n"], ["b\n"], ["c"]], "a\nb\nc".zip)
      assert_equal([["a\n", 1], ["b\n", 2], ["c", 3]],
                   "a\nb\nc".zip([1, 2, 3]))
      assert_equal([[1, nil], [2, nil]], E(1, 2).zip([]))
      Version.greater_or_equal("1.9.0") do
        assert_equal([[1, nil], [2, nil]], E(1, 2).zip(nil))
        assert_equal([[1, 4], [2, 5], [3, 6]], E(1, 2, 3).zip(4..6))
        assert_equal([[1, nil], [2, nil]], E(1, 2).zip(''))
        assert_equal([[1, 'ab'], [2, nil]], E(1, 2).zip('ab'))
        assert_equal([[1, "a\n"], [2, "b\n"], [3, "c"], [4, nil]],
                     E(1, 2, 3, 4).zip("a\nb\nc"))
        assert_equal([[1, ['a', 5]], [2, nil]], E(1, 2).zip({'a'=>5}))
      end
    end
  end
